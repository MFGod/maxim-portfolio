...
export function EditorApp({ fileId }: { fileId: string }) {
  ...
  // Файл изменили снаружи — вставили копию, открыли другой: черновик догоняет
  // хранилище прямо на отрисовке, без лишнего кадра с устаревшим текстом.
  if (node && node.body !== knownBody) {
    setKnownBody(node.body);
    setDraft(node.body);
  }

  // «Сохранено» — это не состояние, а факт: черновик совпал с файлом.
   ...
   *
   * Только для мыши и пера: на сенсорном экране фокус поднял бы клавиатуру
   * поверх половины окна ещё до того, как человек решил печатать.
   */
  useEffect(() => {
    if (!ready) return;
