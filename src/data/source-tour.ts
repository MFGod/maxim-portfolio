...
export const sourceTour: SourceEntry[] = deepFreeze([
  ...
  },
  {
    ...
    responsibilities: [
      ...
      'размеры окна по умолчанию и минимум — только у оконных программ',
      'где показывать: док, рабочий стол',
      'корпус окна: обычный или стеклянный',
    ],
    note: 'Тип разделён надвое. У карты карьеры нет ни размеров окна, ни корпуса: она открывается своей страницей, и обязателен ей только адрес.',
    code: `/** Программа, открывающаяся окном рабочего стола. Такие почти все. */
...
export type ApplicationMeta = WindowApplication | PageApplication;`,
  },
  {
    ...
    note: 'Содержимое подключено статически: объём текста меньше, чем стоит рантайм ленивой загрузки. Заодно всё попадает в серверный HTML.',
