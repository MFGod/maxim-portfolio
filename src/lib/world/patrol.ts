 ...
 *
 * Отдельно от сцены и от three: это арифметика на ломаной, и её единственную
 * тут стоит покрывать тестами по-настоящему. `figures.ts` берёт отсюда числа и
 * раскладывает их по объектам.
 *
 * Группа идёт цепочкой, но считается как одно целое: у строя одно пройденное
 * расстояние, а каждый занимает свою ячейку в нём — первый, второй, третий.
 * Ячейка за человеком закреплена навсегда.
 *
 * Из этого следует главное: **идущие не проходят сквозь друг друга**. Дойдя до
 * конца, группа разворачивается, и задний просто становится ведущим — никто
 * никого не обгоняет и не встречает. Гармошка при этом считается не по всей
 * длине маршрута, а по длине минус длина строя: иначе на развороте хвост
 * свисал бы за край дороги.
 */
...
export function foldDistance(
  ...
  // Отрицательное время бывает у отстающих в строю на первых секундах.
 ...
 *
 * Курс берётся у того звена, по которому идут, а не по разнице соседних точек
 * маршрута: на стыке звеньев второе дало бы скачок в полкадра.
 */
...
export function walkerStep(
  ...
  /*
   * Строй занимает место: если бы каждый шёл «по следу первого с задержкой»,
   * на развороте первый пошёл бы навстречу остальным и прошёл сквозь них.
   * Поэтому идёт не человек по своему времени, а весь строй разом: у группы
   * одно пройденное расстояние, а каждый стоит в своей ячейке строя и остаётся
   * в ней всегда. На развороте группа не перестраивается — просто задний
   * становится ведущим.
   */
