...
describe('содержимое страниц книги', () => {
  it('ни одна страница не выходит пустой', () => {
    // Пустая страница в развороте читается не приёмом, а недоделкой: если
    // раскладка заведёт разворот, которому нечем занять сторону, это видно
    // здесь, а не в кадре.
  ...
  it('текст резюме от языка не зависит', () => {
    /*
     * Решение всего портфолио: переключатель меняет системные подписи, а
     * резюме, проекты и опыт остаются русскими. Книга живёт по тому же
     * правилу, и проверяется это сравнением страниц глав и проектов слово в
     * слово.
     */
  ...
  it('структура страниц одинакова на обоих языках', () => {
    // Разная структура означала бы, что перевод потерял или добавил блок, —
    // а вместе с ним поехали бы и отступы, и мишени ссылок.
  ...
  it('контакты стоят на авантитуле', () => {
    // До конца книги долистает не каждый, а обложку открывает каждый.
