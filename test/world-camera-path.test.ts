...
describe('траектория камеры', () => {
  ...
  it('точку взгляда ведёт вместе с камерой', () => {
    // С точностью до пыли двойной точности: взгляд хранится рысканием,
    // тангажом и дальностью, и на опоре собирается обратно в точку.
  ...
  it('идёт без разрывов: соседние доли дают соседние точки', () => {
    ...
    for (let step = 1; step <= 100; step++) {
      ...
      // Шаг в сотую пути не может быть длиннее десятой его длины.
  ...
  it('движется равномерно: длинный участок не проглатывается', () => {
    // Опоры расставлены с разной длиной участков; при параметризации по номеру
    // точки середина пути пришлась бы на середину списка, а не пути.
 ...
 *
 * Углы важнее положения: камеру, которая едет неровно, посетитель заметит
 * не сразу, а кадр, который швыряет, — мгновенно.
 */
...
describe('взгляд', () => {
  ...
  it('не швыряет там, где точка взгляда прошла бы у самой камеры', () => {
    // Замер до перехода на углы: пик доходил до 729°/с при 6999°/с².
    // Спокойная панорама — 30-60°/с, за 150 кадр уже кидает.
    expect(aiming(around).peak).toBeLessThan(150);
  });

  it('скорость разворота ровная: пик задаёт плавность, а не геометрия', () => {
    // 1.875 — пик `smootherstep`. Всё сверх него набежало бы от кривой.
    expect(aiming(around).ratio).toBeLessThan(2.1);
  });

  it('на входе кадр не швыряет', () => {
    /*
     * Ровности распределения с входа не спрашиваем: его четыре опоры смотрят
     * на выбранные вещи в выбранные моменты, и разворот по пути неравномерен
     * по замыслу. Спрашиваем только пик — он про то, успевает ли глаз.
     */
...
describe('разворот кадра', () => {
  ...
  it('длится дольше, когда кадр разворачивается сильно', () => {
    // Путь короткий, а смотреть камера начинает в другую сторону: раньше такой
    // перелёт укладывался в нижний предел 1.4 с и разворачивал кадр рывком.
